...
int main(void)
{
	...
	ADC_SEQ_initStruct.conv_cnt = 256;					//转换256次
	...
	while(1==1)
	{
		...
		n = 0;
		while(ADC0->GO & ADC_GO_BUSY_Msk)
		{
			if(ADC_DataAvailable(ADC0, ADC_SEQ0))
			{
				if(n < BUFLEN)
					Buffer[n++] = ADC_Read(ADC0, ADC_SEQ0, &chn);
				else
					i = ADC_Read(ADC0, ADC_SEQ0, &chn);
			}
		}
